...
async function format(user) {
  ...
  const dbUser = (await User.findOne({ mobile: user.mobile })).transform();
...
describe('Users API', async () => {
  ...
  describe('POST /v1/users', () => {
    ...
    it('should report error when mobile already exists', () => {
      user.mobile = dbUsers.branStark.mobile;
    ...
    it('should report error when mobile is not provided', () => {
      delete user.mobile;
  ...
  describe('GET /v1/users', () => {
    ...
    it('should filter users', () => {
      return request(app)
        .get('/v1/users')
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .query({ mobile: dbUsers.jonSnow.mobile })
        .expect(httpStatus.OK)
        .then((res) => {
          delete dbUsers.jonSnow.password;
          const john = format(dbUsers.jonSnow);
  ...
  describe('GET /v1/users/:userId', () => {
    ...
    it('should report error when logged user is not the same as the requested one', async () => {
      const id = (await User.findOne({ mobile: dbUsers.branStark.mobile }))._id;
  ...
  describe('PUT /v1/users/:userId', () => {
    ...
    it('should report error when mobile is not provided', async () => {
      const id = (await User.findOne({}))._id;
      delete user.mobile;
    ...
    it('should not replace the role of the user (not admin)', async () => {
      const id = (await User.findOne({ mobile: dbUsers.jonSnow.mobile }))._id;
      const role = 'admin';
    ...
    it('should update user', async () => {
      ...
      return request(app)
        ...
        .then((res) => {
          ...
          expect(res.body.mobile).to.be.equal(dbUsers.branStark.mobile);
        });
    });
  ...
  describe('GET /v1/users/profile', () => {
    ...
    it('should report error without stacktrace when accessToken is expired', async () => {
      ...
      // move clock forward by days
      clock.tick((JWT_EXPIRATION * 60230230000) + 60000000);
