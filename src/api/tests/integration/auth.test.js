...
describe('Authentication API', () => {
  ...
  describe('POST /v1/auth/register', () => {
    ...
    it('should report error when mobile already exists', () => {
      return request(app)
        .post('/v1/auth/register')
        .send(dbUser)
        ...
        .then((res) => {
          ...
          expect(field).to.be.equal('mobile');
          expect(location).to.be.equal('body');
          expect(messages).to.include('"mobile" already exists');
        });
    });

    it('should report error when the mobile provided is not valid', () => {
      user.mobile = 'this_is_not_a_mobile_number';
      return request(app)
        .post('/v1/auth/register')
        .send(user)
        ...
        .then((res) => {
          ...
          expect(field).to.be.equal('mobile');
          expect(location).to.be.equal('body');
          expect(messages).to.include('"mobile" must be a number');
        });
    });

    it('should report error when mobile and password are not provided', () => {
      return request(app)
        .post('/v1/auth/register')
        .send({})
        .expect(httpStatus.BAD_REQUEST)
        .then((res) => {
          ...
          expect(field).to.be.equal('mobile');
          expect(location).to.be.equal('body');
          expect(messages).to.include('"mobile" is required');
        });
    });
  });

  describe('POST /v1/auth/login', () => {
    it('should return an accessToken and a refreshToken when mobile and password matches', () => {
      return request(app)
        .post('/v1/auth/login')
        .send(dbUser)
    ...
    it('should report error when mobile and password don\'t match', () => {
      dbUser.password = 'xxx';
      return request(app)
        .post('/v1/auth/login')
        .send(dbUser)
        ...
        .then((res) => {
          ...
          expect(message).to.be.equal('Incorrect mobile number or password');
        });
    });
  });
  ...
  describe('POST /v1/auth/refresh-token', () => {
    it('should return a new accessToken when refreshToken and mobile match', async () => {
      await RefreshToken.create(refreshToken);
      return request(app)
        .post('/v1/auth/refresh-token')
        .send({ mobile: dbUser.mobile, refreshToken: refreshToken.token })
        .expect(httpStatus.OK)
        .then((res) => {
    ...
    it('should report error when mobile and refreshToken don\'t match', async () => {
      await RefreshToken.create(refreshToken);
      return request(app)
        .post('/v1/auth/refresh-token')
        .send({ mobile: user.mobile, refreshToken: refreshToken.token })
        .expect(httpStatus.UNAUTHORIZED)
        .then((res) => {
          const { code } = res.body;
          const { message } = res.body;
          expect(code).to.be.equal(401);
          expect(message).to.be.equal('Incorrect mobile number or refreshToken');
        });
    });

    it('should report error when mobile and refreshToken are not provided', () => {
      return request(app)
        .post('/v1/auth/refresh-token')
        .send({})
        .expect(httpStatus.BAD_REQUEST)
        .then((res) => {
          ...
          expect(field1).to.be.equal('mobile');
          expect(location1).to.be.equal('body');
          expect(messages1).to.include('"mobile" is required');
    ...
    it('should report error when the refreshToken is expired', async () => {
      ...
      return request(app)
        .post('/v1/auth/refresh-token')
        .send({ mobile: dbUser.mobile, refreshToken: expiredRefreshToken.token })
        .expect(httpStatus.UNAUTHORIZED)
        .then((res) => {
