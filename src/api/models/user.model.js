...
userSchema.method({
  transform() {
    const transformed = {};
    const fields = ['id', 'name', 'email', 'mobile', 'picture', 'role', 'createdAt'];
...
userSchema.statics = {
  ...
  async findAndGenerateToken(options) {
    ...
    if (password) {
      ...
      err.message = 'Incorrect mobile number or refreshToken';
    }
   ...
   */
  list({
    page = 1, perPage = 30, name, email, mobile, role,
  }) {
  ...
  async oAuthLogin({
    service, id, email, name, picture, mobile
  }) {
    const user = await this.findOne({ $or: [{ [`services.${service}`]: id }, { mobile }] });
    if (user) {
    ...
    return this.create({
      services: { [service]: id }, email, password, name, picture, mobile
    });
  },
};
